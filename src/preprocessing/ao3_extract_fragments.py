"""
Extract only emotionally intense 40-60 word fragments from AO3 stories
...
def main():
    ...
    input_path = RAW_DATA_DIR / "ao3_stories.parquet"
    ...
    for i, story in enumerate(tqdm(stories, desc="Extracting fragments"), 1):
        ...
        if i % 5 == 0:
            batch_output_path = PROCESSED_DATA_DIR / f"ao3_fragments_batch_{batch_num}.parquet"
            batch_df = pd.DataFrame(batch_fragments)
            batch_df.to_parquet(batch_output_path, index=False)
    ...
    output_path = PROCESSED_DATA_DIR / "ao3_fragments.parquet"
    fragment_df = pd.DataFrame(all_fragments)
    print('Saving final DF...')
    fragment_df.to_parquet(output_path, index=False)
