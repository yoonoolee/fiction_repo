...
def train_and_upload(dataset_name, output_name, num_epochs=1):
    ...
    training_args = SFTConfig(
        ...
        learning_rate=2e-5,  # Lower LR for Qwen (was 1e-4)
        ...
        eval_strategy="steps",  # Evaluate during training to monitor overfitting
        eval_steps=100,  # Check val loss every 100 steps
        report_to="none",
    ...
    try:
        ...
        # Clone llama.cpp if not present (needed for Qwen GGUF conversion)
        if not os.path.exists("llama.cpp"):
            ...
            subprocess.run(["pip", "install", "-r", "llama.cpp/requirements.txt", "--quiet"], check=True)

        gguf_path = f"outputs/{output_name}.gguf"
        print(f">>> Creating GGUF file: {gguf_path}")

        # Use llama.cpp convert script (supports Qwen models)
        subprocess.run([
        ...
        print(f">>> Uploading GGUF to HuggingFace: {HF_USERNAME}/{output_name}-gguf")
        api = HfApi()
        api.create_repo(repo_id=f"{HF_USERNAME}/{output_name}-gguf", token=HF_TOKEN, exist_ok=True)
        api.upload_file(
            path_or_fileobj=gguf_path,
            path_in_repo=f"{output_name}.gguf",
            repo_id=f"{HF_USERNAME}/{output_name}-gguf",
            token=HF_TOKEN,
        )
        print(f">>> ✓ GGUF uploaded! Download from: https://huggingface.co/{HF_USERNAME}/{output_name}-gguf")

    except Exception as e:
        print(f">>> GGUF conversion failed: {e}")
        print(f">>> You can convert manually using llama.cpp later:")
        print(f">>>   python llama.cpp/convert_hf_to_gguf.py {merged_dir} --outfile {output_name}.gguf --outtype q4_k_m")
...
configs = [
    ("combined", "qwen-2.5-7b-combined", 1),
]
